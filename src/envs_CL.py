...
# =========================================================
# 🔥 NEW: Curriculum learning by track seeds
# =========================================================
class CurriculumSeedWrapper(gym.Wrapper):
    """
    Selects track seeds from a predefined list at every reset.
    Used for curriculum learning by track complexity.
    """
    def __init__(self, env: gym.Env, seeds: List[int]):
        super().__init__(env)
        assert len(seeds) > 0, "CurriculumSeedWrapper requires a non-empty seed list"
        self.seeds = seeds
...
# =========================================================
# Environment factory (MINIMAL MODIFICATION)
# =========================================================
def make_env_fn(
    env_id: str,
    seed: int,
    ...
    curriculum_seeds: Optional[List[int]] = None,  # 🔥 NEW
) -> Callable[[], gym.Env]:
    ...
    def _make() -> gym.Env:
        ...
        if curriculum_seeds is not None:
            env = CurriculumSeedWrapper(env, curriculum_seeds)
        else:
            env.reset(seed=seed)
...
# =========================================================
# Vectorized environment (MINIMAL MODIFICATION)
# =========================================================
def make_vec_env(
    env_id: str,
    seed: int,
